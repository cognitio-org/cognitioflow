...
- `llm.py` exposes `client()`, `resolve(model_id) -> str`, `usage(m, task) -> dict`, `PROVIDER`, `catalogue()`. `run.py` imports only these; it never imports `anthropic` and never reads `OPENROUTER_KEY` or `ANTHROPIC_API_KEY` itself (`config`, `check_env` and the startup banner ask `llm`).
...
- `LLM_PROVIDER=openrouter`: `client()` returns `anthropic.Anthropic(base_url="https://openrouter.ai/api", auth_token=os.environ["OPENROUTER_KEY"], default_headers={"HTTP-Referer": "https://github.com/cognitio-org/cognitioflow", "X-OpenRouter-Title": "CognitioFlow"})`. `resolve` maps a Claude id without a slash to OpenRouter's spelling (`claude-sonnet-4-6 → anthropic/claude-sonnet-4.6`, `claude-sonnet-5 → anthropic/claude-sonnet-5`) so the same `.env` works for both backends; ids with a slash pass through. No OpenRouter-specific body fields are sent in v1 (no `provider` preferences, no `models` fallback list).
...
- `check_env.py`: in production require `OPENROUTER_KEY` when `LLM_PROVIDER=openrouter`, else `ANTHROPIC_API_KEY`; refuse an unknown provider value.
- `.env.local.example`: `LLM_PROVIDER`, `OPENROUTER_KEY`, `CF_EXTRA_MODELS` documented next to the model block. Phase 5's `infra/setup.sh` and deploy workflow gain an optional sixth secret `OPENROUTER_KEY` and the `LLM_PROVIDER` env var; the cloud default stays `anthropic` until Matej flips it.
...
- `check_env`: production with `LLM_PROVIDER=openrouter` and no `OPENROUTER_KEY` is fatal; the Anthropic path is unchanged.
...
## How to see it
```
LLM_PROVIDER=openrouter OPENROUTER_KEY=sk-or-… make dev      # then open http://localhost:8000
```
