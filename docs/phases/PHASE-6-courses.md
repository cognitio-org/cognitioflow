...
## Decisions [Matej]
- **Property Law details still owed:** exam format, WG tutor name, permitted materials. These go in through the new course dialog, not into code.
- **Course prompt template:** confirm the field list below or add to it.
...
Add `brief JSONB DEFAULT '{}'` to `courses`. Fields: `lecturer`, `wg_tutor`, `textbook`, `exam_format`, `permitted_materials`, `authority_order` (list), `method` (free text, e.g. Villanueva's three steps), `provenance_tags` (list), `notes` (free text). `tutor_prompt` becomes derived: `compile_prompt(brief)` renders it from a template so every course's prompt has the same shape as the current EU one. If `brief` is empty, fall back to the stored `tutor_prompt` (keeps existing courses working).
